...
Code GetCode(void *codeSet, int code) {                         
   ...
   if (++(current->getCodeCount) == 1) {                   
      ...
      unsigned char *ptr = current->fullCode + size - 1;          
...
void DestroyCodeSet(void *codeSet) {
   CodeSet *myCodeSet = (CodeSet *)codeSet;                           
   CodeEntry *temp = myCodeSet->array;                                                                                                                                                    while (temp < myCodeSet->ndx + myCodeSet->array) {
         if (temp->getCodeCount) {                                                                                                                                                              free(temp->fullCode);
         }
         temp++;
      }
